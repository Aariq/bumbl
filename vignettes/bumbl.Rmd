...
***TODO***: address convergence warnings from glm.fit
...
```{r}
library(ggplot2)
p <- autoplot(out, colony = c("17", "104", "20"))
p + theme_bw()
```
...
out3.1 %>% filter(colony %in% c("104", "17", "20"))
```

We can see that colony 20 has a much lower R^2^ value than colonies 104 and 17, which matches our expectations from plotting the observed and fitted values.
